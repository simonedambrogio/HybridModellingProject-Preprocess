...
def prepare_wbrain_fmap(config, subject: int, session: int, input_dir: str, output_dir: str):
    print("Reorienting boldwb to standard...")
    reorient2standard(config, subject, session, "boldwb", input_dir=input_dir, output_dir=output_dir)
    print("Reorienting fmapph to standard...")
    reorient2standard(config, subject, session, "fmapph", input_dir=input_dir, output_dir=output_dir)
    print("Reorienting fmapmg to standard...")
    reorient2standard(config, subject, session, "fmapmg", input_dir=input_dir, output_dir=output_dir)
...
if __name__ == "__main__":
    ...
    prepare_wbrain_fmap(config, args.subject, args.session, args.input_dir, args.output_dir)
