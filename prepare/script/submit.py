...
def biascorrect_submit(config, subject: int, session: int, input_dir: str, output_dir: str, job_id: int = None, submit: str = "slurm"):
    assert submit in ["slurm", "sge"], f"Submit method {submit} not supported"
    if submit == "slurm":
        biascorrect_submit_slurm(config, subject, session, input_dir, output_dir, job_id)
    elif submit == "sge":
        biascorrect_submit_sge(config, subject, session, input_dir, output_dir, job_id)
    else:
        raise ValueError(f"Submit method {submit} not supported")
