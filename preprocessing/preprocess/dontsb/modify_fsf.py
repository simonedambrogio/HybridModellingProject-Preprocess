import os, argparse
from preprocessing.utils import _sbjname, _sesname, get_fsf_dir
...
def modify_fsf(
    ...
    if input_dir is None:
        input_dir = os.path.join(config['prepare']['paths']['output'], sbjname, sesname)
    if output_dir is None:
        output_dir = os.path.join(config['preprocess']['paths']['output'], sbjname, sesname)
        
    func_dir = os.path.join(input_dir, "func_biascorr.nii.gz")
    ...
    fmap_rads_dir = os.path.join(input_dir, "fmap_rads.nii.gz")
    fmap_mag_dir = os.path.join(input_dir, "fmap_mag_brain.nii.gz")
    whole_func_dir = os.path.join(input_dir, "expanded_func_brain.nii.gz")
    struct_brain_dir = os.path.join(input_dir, "synthstrip_biascorr_brain.nii.gz")
    alternative_mask_dir = os.path.join(input_dir, "funcmask.nii.gz")
    
    # Prepare substitution dictionary
    img = nib.load(func_dir)
    subs = {
        'outputdir': os.path.join(output_dir, feat_name),
        'tr': get_tr(img),
        'te': 27,
    ...
    fsf_dir = save_fsf(output_dir, fsf_content, fsf_name)
